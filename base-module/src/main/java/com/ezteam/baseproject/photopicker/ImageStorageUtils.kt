...
object ImageStorageUtils {
        ...
        cursor?.let { data ->
            if (data.moveToFirst()) {
                do {
                    ...
                    val dateAdded: Long = data.getLong(data.getColumnIndexOrThrow(MediaColumns.DATE_ADDED))
                    if (size < 1) continue
                    ...
                    if (!directories.contains(photoDirectory)) {
                        ...
                        directories[directories.indexOf(photoDirectory)].addPhoto(imageId, path, dateAdded)
                    }
                    path?.let {
    ...
    fun getPhotoDirs(context: Context, photoListener: (Photo) -> Unit){
        ...
        cursor?.let { data ->
            if (data.moveToFirst()) {
                do {
                    val imageId: Long = data.getLong(data.getColumnIndexOrThrow(BaseColumns._ID))
                    val bucketId: String? =
                        data.getString(data.getColumnIndexOrThrow(MediaColumns.BUCKET_ID))
                    val name: String? =
                        data.getString(data.getColumnIndexOrThrow(MediaColumns.BUCKET_DISPLAY_NAME))
                    ...
                    val photo = Photo(imageId, path ?: "", bucketId ?: "", name ?:"", false, dateAdded)
                    photoListener(photo)
                } while (data.moveToNext())
            }
        }
