...
public class SplitView extends LinearLayout implements OnTouchListener {
    ...
    public SplitView(Context context, AttributeSet attrs) {
        ...
        minLeft = viewAttrs.getDimensionPixelSize(R.styleable.SplitView_minLeft, 0);
        minRight = viewAttrs.getDimensionPixelSize(R.styleable.SplitView_minRight, 0);
        minTop = viewAttrs.getDimensionPixelSize(R.styleable.SplitView_minTop, 0);
        minBottom = viewAttrs.getDimensionPixelSize(R.styleable.SplitView_minBottom, 0);
        viewAttrs.recycle();
...
//                    maximizeSecondaryContent();
            ...
            if (dragFinishListener != null)
                dragFinishListener.onFilish();
            return true;
        } else if (me.getAction() == MotionEvent.ACTION_MOVE) {
            if (getOrientation() == VERTICAL) {
                if (me.getRawY() > minTop && getHeight() - me.getRawY() > minBottom) {
                    setPrimaryContentHeight((int) (me.getRawY() - mPointerOffset));
                }
            } else {
                if (me.getRawX() > minLeft && getWidth() - me.getRawX() > minRight) {
    ...
    public boolean setPrimaryContentWidth(int newWidth) {
