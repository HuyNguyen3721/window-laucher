...
import com.ezteam.windowslauncher.utils.center.ManualFilterUtil

class BroadCastManual : BroadcastReceiver() {
    lateinit var lisDataControl: MutableList<ItemControl>
    lateinit var adapter: AdapterControl
    override fun onReceive(context: Context?, intent: Intent?) {
        intent?.let {
            context?.let { c ->
                if (c.resources.getString(R.string.ACTION_MAMUAL) == it.action) {
                    for (item in lisDataControl) {
                        if (item.name == c.resources.getString(R.string.manual)) {
                            item.isEnable = ManualFilterUtil.detectManual(c)
